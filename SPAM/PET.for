...
!         ------------------------
     ...
     &        EO, ET0)                                  !Output
!         ------------------------
          !Modified Priestly-Taylor potential evapotranspiration
          ! for soybean based on Mercau et el. 2007
          CASE ('A') 
            CALL PETPTM(
...
C=======================================================================
C  PETPTM, Subroutine, J.T. Ritchie
C  Calculates Priestly-Taylor potential evapotranspiration
C  incorporating the modifications by Mercau et al. 2007
C-----------------------------------------------------------------------
C  REVISION HISTORY
C  ??/??/19?? JR  Written
C  11/04/1993 NBP Modified
...
!  07/24/2006 CHP Use MSALB instead of SALB (includes mulch and soil 
!                 water effects on albedo)
!  06/15/2021 AB  Empirical modification to reduce the bias at high LAIs
!-----------------------------------------------------------------------
!  Called by:   WATBAL
!  Calls:       None
C=======================================================================
...
!         developed.
      ...
      ! Linear correction of the equation
      ! empirical correction factor in the model code that multiplies 
      ! potential evapotranspiration by 1.22 when LAI is >= 4.0. 
      ! The correction factor decreases linearly to 1 when LAI = 0.
      IF (XHLAI .GE. 4.0) THEN
      	EO = EO*1.22
      ELSE 
      	EO = EO*(0.055*XHLAI+1)
      ENDIF
